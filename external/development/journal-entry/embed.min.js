(function(l){"use strict";const r=new Map,m=(()=>{const t=document.getElementsByTagName("script");for(let s=t.length-1;s>=0;s-=1){const o=t[s].src;if(o)try{const e=new URL(o);if(e.pathname.endsWith("/embed.min.js")||e.pathname.endsWith("/embed.js"))return e.origin+e.pathname.replace(/\/embed(\.min)?\.js$/,"")}catch{continue}}return""})();function h(t){if(!t.element)return console.error("[RedwoodJournalAssessment] Missing container element"),null;const s=r.get(t.element);s&&(s.remove(),r.delete(t.element));const o=t.options?.baseUrl||m,e=t.version||"latest",n=e==="dev",u=e!=="latest"&&e!=="dev"?`v${e}`:e,a=new URL(n?"/":`${o}/${u}/index-embed.html`,n?window.location.origin:void 0);a.searchParams.set("embed","true"),a.searchParams.set("type",t.type);const y=t.options?.reportBaseUrl||window.location.href;a.searchParams.set("reportBaseUrl",y),a.searchParams.set("autoRedirect",String(t.options?.autoRedirect??!0));const i=document.createElement("iframe");return i.src=a.toString(),i.style.cssText=`
    width: 100%;
    min-height: 720px;
    border: none;
    display: block;
